...
function init() {
    d3.json("../Resources/samples.json").then((importedData) => {
        ...
        // bar
        sample_values = firstSample.sample_values.slice(0, 10).reverse();
        otu_Ids = firstSample.otu_ids.slice(0, 10).reverse().map(id => "OTU " + id);
        otu_Labels = firstSample.otu_labels.slice(0, 10).reverse();
